...
- check-documentation: runs on pull requests to master and dev, plus
  manual dispatch
- r-build-check: runs on pull requests to master and dev, plus manual
  dispatch
- R-CMD-check-bioc: runs on pull requests to master, main, and dev, plus
  manual dispatch
...
Use the URL below (replace `BRANCHNAME` with the source branch name)
when opening promotion PRs:
...
The workflow `validate-master-pr-checklist` will fail unless all
required checklist items are present and checked (`[x]`) in the PR body.
...
- `[release-master] post-merge push plan to \`bioc-upstream/devel\`
  confirmed\`
