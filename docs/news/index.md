...
- circos_ligand_receptor will not fail in cases where the rl_map of a
  ...
  not present in the expression matrix. A message is returned if ligands
  from the rl_map had to be excluded from the plot
...
- refactorization of circos_ligand_receptor function to remove use of
  ...
  from a domino object into a data frame for plotting a receptor circos
  plot. Component functions for creating the data frame of ligand
  expression centered on a single receptor and to render this data frame
  as a circos plot are now included in the package as
...
- Resolved issue where if create_domino was run with complexes=TRUE and
  no complexes were found to have active signaling, the full signaling
  matrix would be replaced by a NULL value.
...
- Version number update to signify acceptance to bioconductor in release
  3.20
...
- Disabled exact p-value computation for the correlation test between
  ...
  due to tied ranks during Spearman correlation calculation
...
- Implemented caching of example data by BiocCache to meet package size
  limits
...
- Corrected BiocCheck notes pertaining to coding practices including
  ...
  usage of seq_len or seq_along in place of seq, and usage of vapply in
  place of sapply
...
- Uniform formats for inputs of receptor - ligand interaction databases,
  transcription factor activity features, and regulon gene lists for
  operability with alternative databases and transcription factor
  activation inference methods
...
- Assessment of complex ligand expression as the mean of component gene
  expression for plotting functions
- Minimum threshold for the percentage of cells in a cluster expressing
  a receptor gene for the receptor to be called active within the
  cluster
...
- Chord plot of ligand expression targeting a specified receptor where
  ...
  cell cluster
- Signaling networks showing only outgoing signaling from specified cell
  clusters
...
- Added host option for gene ortholog conversions using
  [biomaRt](https://github.com/Huber-group-EMBL/biomaRt) for access to
  maintained mirrors
- Transcription factor - target linkages are now properly stored so that
  receptors in a transcription factor’s regulon are excluded from
  linkage
...
- [`create_domino()`](https://FertigLab.github.io/dominoSignal/reference/create_domino.md)
  can be run without providing a regulon list
- References to the host GitHub repository have been updated to
  [Elisseeff-Lab](https://github.com/Elisseeff-Lab/domino)
