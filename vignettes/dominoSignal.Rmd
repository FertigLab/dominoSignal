...
```{r}
#| fig.alt: >
#|  Heatmap showing feature activation for cells ordered by cluster.
...
```{r}
#| fig.alt: >
#|  Network plot with columns corresponding to ligands, receptors, and transcription factors
#|  for signaling to dendritic cells.
...
#| fig.alt: >
#|  Network plot with columns corresponding to ligands, receptors, and transcription factors
#|  showing signaling to dendritic cells from the CD14 monocyte population.
...
```{r, out.width = "95%"}
#| fig.alt: >
#|  Heatmap showing ligand expression from each sending population to receiving receptors in
...
#| fig.alt: >
#|  A circos plot with outer arcs corresponding to clusters and inner arcs corresponding to ligands
#|  directed to receptor CD74. Arcs represent mean expression of the ligands.
circos_ligand_receptor(pbmc_dom, receptor = "CD74")
```

## Example run

Here is an example of the complete dominoSignal workflow with default settings using the PBMC data set. We start from the `pbmc` SingleCellExperiment object, the SCENIC results, and the CellPhoneDB files. In this example, we focus on signaling to the CD14 monocyte cluster.
...
# Infer network
pbmc_dom <- create_domino(rl_map = rl_map, features = auc_in, counts = counts,
