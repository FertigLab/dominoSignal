...
#'   cluster. If left as NULL then default ggplot colors will be generated.
...
#' @param ... Other parameters to pass to  [ComplexHeatmap::Heatmap()].
#'   Note that to use the 'main' parameter of  [ComplexHeatmap::Heatmap()]  you must set title = FALSE
...
feat_heatmap <- function(
    ...
    if (ann_cols) {
        ...
        names(ac[[1]]) <- NULL
        if (is.null(cols)) {
            cols <- ggplot_col_gen(nlevels(cl))
    ...
    if (!isFALSE(title) && !isFALSE(ann_cols)) {
        Heatmap(
            mat,
            name = "feature\nactivity",
    ...
    } else if (isFALSE(title) && !isFALSE(ann_cols)) {
        Heatmap(
            mat,
            name = "feature\nactivity",
            top_annotation = feat_anno,
...
#'   if making a boolean heatmap.
#' @param title Either a string to use as the title or a boolean describing whether to include a title.
#'   In order to pass the 'main' parameter to  [ComplexHeatmap::Heatmap()]  you must set title to FALSE.
#' @param feats Either a vector of features to include in the heatmap or 'all' for all features.
#'   If left NULL then the features selected for the signaling network will be shown.
#' @param recs Either a vector of receptors to include in the heatmap or 'all' for all receptors.
#'   If left NULL then the receptors selected in the signaling network connected to the features plotted will be shown.
#' @param mark_connections Boolean indicating whether to add an 'x' in cells where there is a connected receptor
#'   or TF. Default FALSE.
#' @param ... Other parameters to pass to  [ComplexHeatmap::Heatmap()] .
#'   Note that to use the 'main' parameter of  [ComplexHeatmap::Heatmap()]  you must set title = FALSE and to
...
cor_heatmap <- function(
    ...
    if (!isFALSE(title) && mark_connections) {
        Heatmap(
            mat,
            name = "rho",
            column_title = title,
