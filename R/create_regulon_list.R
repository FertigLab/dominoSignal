...
#'
#' Generates a list of transcription factors and the genes targeted by the transcription factor as part of
#'  their regulon inferred by pySCENIC
#'
#' @param regulons Data frame or file path to the table of the output of the ctx function from pySCENIC
#' @return A list where names are transcription factors and the stored values are character vectors of genes
#'   in the inferred regulons
...
create_regulon_list_scenic <- function(regulons) {
    ...
    TF_targets <- lapply(TFS, function(tf) {
        ...
        target_genes <- lapply(targets, function(x) {
            split_targs <- unlist(strsplit(x, "", fixed = TRUE))
            ...
            split_targs_pre <- split_targs[nzchar(split_targs, keepNA = TRUE)]
